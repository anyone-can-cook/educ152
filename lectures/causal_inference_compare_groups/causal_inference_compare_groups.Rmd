...
subtitle: "Fundamentals of causal inference and comparing two groups"
author: 
date: 
urlcolor: blue
output: 
...
2. Observational (i.e., non-experimental) "selection on observables" designs
	...
	- Matching estimators (e.g., propensity score matching)
...
Unit causal effect: $\tau_i = Y_i(1) - Y_i(0)$
...
- $E[Y_i|T_i=1]$
	- Expected value of observed test score, given that student is assigned to the treatment ("small" class size)
...
| 6  | 80  | **70**  | ?  |
| 7  | **45**  | 45  | ?  |
| **Avg. observed**  | **41.66**  | **53.75**  | **-12.08** |
...
- rather, treatment assignment $T_i$ just affects which potential outcome we observe
  - If $T_i=1$ then the observed outcome, $Y_i$, is the treated potential outcome, $Y_i(1)$
  - If $T_i=0$ then the observed outcome, $Y_i$, is the untreated potential outcome, $Y_i(0)$
...
However, knowing expected values -- e.g., $E[Y_i|T_i=1]$, the expected value of $Y_i$ amongst treated units -- requires taking an infinite number of samples
...
- $\hat{ATE} = \hat{\tau} = \bar{Y}_{treatment} - \bar{Y}_{control}$

<br>

- $\hat{ATE} = \frac{\sum_{i=1}^{n_{t=1}} Y_{i,t=1}}{n_{t=1}} - \frac{\sum_{i=1}^{n_{t=0}} Y_{i,t=0}}{n_{t=0}}$, where:
  - $Y_{i,t=1}$: observed outcome for a unit $i$ who received the treatment
  - $Y_{i,t=0}$: observed outcome for a unit $i$ who received the control
  - $n_{t=1}$: number of units that received the treatment 
  - $n_{t=0}$: number of units that received the control
...
- sample mean reading score for untreated units = `r round(mean(df_stark$read[df_stark$treatment==0], na.rm = TRUE), digits=2)`
...
We'll use our "user defined functions" (loaded above) to visually explain concepts for hypothesis testing about population mean of two groups

- these functions have been updated/modified as of 4/15/2021
- try messing around with these functions on your own (e.g., try running them with a different variable and play around with function argument values)
...
`plot_t_distribution()` function generates sampling distribution (with t-test values) assuming null hypothesis is correct

- syntax and default values: 
  - `plot_t_distribution(data_vec1, data_vec2 = NULL, data_df = NULL, data_var = NULL, group_var = NULL, group_cat = NULL, mu = 0, alpha = 0.05, alternative = 'two.sided', plot_title = '', shade_rejection = T, shade_pval = T, stacked = F)`
...
# SINGLE POPULATION
  ...
  plot_t_distribution(data_df = df_ipeds_sample, data_var = 'coa_grad_res', mu = 29000)
    #plot_t_distribution(df_ipeds_sample$coa_grad_res, mu = 29000) # same as above
...
# TWO POPULATIONS  
  ...
  plot_t_distribution(data_df = df_ipeds_sample, data_var = 'tuitfee_grad_res',group_var = 'control', group_cat = c(1, 2), shade_pval = F)
  #plot_t_distribution(data_df = df_ipeds_sample, data_var = 'tuitfee_grad_nres',group_var = 'control', group_cat = c(1, 2), shade_pval = F)
...
**Hypothesis about reading score of score of "small" vs. "big" class size**
...
**Hypothesis about resident grad tuition+fees at private vs. public universities**

*note*: for private universities, resident price is the same as non-resident price

null hypothesis ($H_0$)

- $H_0: \mu_{Y_{{priv}}} = \mu_{Y_{{pub}}}$
- (in words): $H_0:$ the population mean price of resident tuition+fees at private universities, $\mu_{Y_{{priv}}}$, is the same as population mean price of resident tuition+fees at public universities ($\mu_{Y_{{pub}}}$)

Two-sided alternative hypothesis ($H_a$)

- $H_0: \mu_{Y_{{priv}}} \ne \mu_{Y_{{pub}}}$
- (in words): $H_0:$ the population mean price of resident tuition+fees at private universities, $\mu_{Y_{{priv}}}$, is not equal to the population mean price of resident tuition+fees at public universities ($\mu_{Y_{{pub}}}$)

<br>

Since IPEDS has data on the whole population, we can plot the population distributiion, distribution of single sample, and the sampling distribution

- *note*: when testing hypothesis about a single population mean, we were concerned with the sampling distribution of the sample mean, $\bar{Y}$
- when testing hypothesis about whether two population means are the same/different, we are concerned with the sampling distribution of the difference between sample means, ($\bar{Y}_{group2} - \bar{Y}_{group1}$)

```{r}
  plot_distribution(data_df = df_ipeds_pop, data_var = 'tuitfee_grad_res', group_var = 'control',group_cat = c(2, 1), sampling_dist = F, pop_labels = c('Private', 'Public'), plot_title = 'Population distribution') +
  plot_distribution(data_df = df_ipeds_sample, data_var = 'tuitfee_grad_res', group_var = 'control',group_cat = c(2, 1), sampling_dist = F, pop_labels = c('Private', 'Public'), plot_title = 'Single sample distribution') +   plot_distribution(data_df = df_ipeds_pop, data_var = 'tuitfee_grad_res', group_var = 'control',group_cat = c(2, 1), sampling_dist = T, pop_labels = c('Private', 'Public'), plot_title = 'Sampling distribution') +
  plot_layout(ncol = 1)
```

<br>

Usually we cannot create the true sampling distribution because we do not have data on the entire population. So we test hypotheses about equality of two groups by creating the sampling distribution of the difference between sample means, ($\bar{Y}_{group2} - \bar{Y}_{group1}$) under the assumption that $H_0$ is true

- that is, the sampling distribution under the assumption that $\mu_{Y_{{group2}}} = \mu_{Y_{{group1}}}$ or -- equivalently $\mu_{Y_{{group2}}} - \mu_{Y_{{group1}}}=0$)

```{r}
  plot_distribution(data_df = df_ipeds_pop, data_var = 'tuitfee_grad_res', group_var = 'control',group_cat = c(2, 1), sampling_dist = F, pop_labels = c('Private', 'Public'), plot_title = 'Population distribution') +
  plot_distribution(data_df = df_ipeds_sample, data_var = 'tuitfee_grad_res', group_var = 'control',group_cat = c(2, 1), sampling_dist = F, pop_labels = c('Private', 'Public'), plot_title = 'Single sample distribution') +
  plot_t_distribution(data_df = df_ipeds_sample, data_var = 'tuitfee_grad_res',group_var = 'control', group_cat = c(2, 1), shade_pval = F, stacked = T) +
  plot_layout(ncol = 1)

```

Just as in hypothesis testing about a single population mean, the value of the t-statistic shows how many standard errors the observed point estimate is away from the null population parameter (i.e., $\mu_{Y_{{group2}}} - \mu_{Y_{{group1}}}==0$). 

- since the sampling distribution assuming $H_0$ is normally distributed, it follows the "empirical rule" 
- assuming $H_0$ is true, 95% of point estimates should fall within 2 standard errors (i.e., $-2<t<2$) 99% of point estimates would fell within 3 standard errors ((i.e., $-3<t<3$))
- above, we got a t-statistic higher than 6. very unlikely to observe this if $H_0$ is true. therefore, we conclude that $H_0$ is not true.


## Test statistic

Going back to the Tennessee STAR example, let's restate $H_0$ and $H_a$
...
Above, we observed a p-value of less than `0.01`, indicating it would be very unlikely to observe the pooint estimate we observed ($\bar{Y}_{treated}-\bar{Y}_{control}=$ `r round(mean(df_stark$read[df_stark$treatment==1], na.rm = TRUE) - mean(df_stark$read[df_stark$treatment==0], na.rm = TRUE), digits = 2)`) under the assumption that $H_0$ is true

<br>

Below, we visually show the sampling distribution assuming $H_0$ is true

- note that the blue dotted line associated with the t-statistic/p-value far exceeds the red dotted line, which is associated with the threshold for rejection of $H_0$
```{r}
plot_t_distribution(data_df = df_stark, data_var = 'read',group_var = 'treatment', group_cat = c(1, 0), shade_pval = F)
```
