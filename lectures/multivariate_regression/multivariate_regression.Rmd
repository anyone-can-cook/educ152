...
# query data frame df_els_stu_all
  lookfor(data=df_els_stu_all, 'how far in school',labels = TRUE, ignore.case = TRUE, details = TRUE)
...
<br>
__Identify a couple of "control variables" (often referred to "covariates") to add to our model__ 

We want to recreate "experimental conditions" (students randomly assigned to internship)

- Our goal is: after controlling for covariates, students who participate in internships are, on average, the same as students who do not participate in internships with respect to 2011 earnings, except for participation in internship
...
- [continuous] Let's pretend that `bytxrstd` (standardized reading test score) is a measure of high school student achievement
  - Does high school student achievement affect subsequent earnings? (omitted variable bias condition 1)
    - Yes!
  - Might high school student achievement have a relationship with whether a student has an internship in college? (omitted variable bias condition 2)
...
- [categorical control] sex (`f1sex`; with values "Male" and "Female")
  ...
  - Might sex have a relationship with whether a student has an internship in college? (omitted variable bias condition 2)
...
- [categorical control] race (`f1race_v2`)
  ...
  - Might race have a relationship with whether a student has an internship in college? (omitted variable bias condition 2)
...
- Population linear regression model (multivariate regression)
  - $Y_i = \beta_0 + \beta_1 X_{1i}+ \beta_2 X_{2i}+ \beta_3 X_{3i}+ \beta_4 X_{4i}+ \beta_5 X_{5i}+ \beta_6 X_{6i}+ \beta_7 X_{7i}+ \beta_8 X_{8i}+ u_i$, where:
    - $Y_i$: is earnings for student $i$ in 2011 (in dollars)    
...
- For high school reading test score $X_{2i}$, $\hat{\beta_2}=$ `r format(round(summary(intern_mod2)$coefficients[3,1], digits =0),big.mark = ',')`
  - a one-point increase in high school reading test score is associated with a `r format(round(summary(intern_mod2)$coefficients[3,1], digits =0),big.mark = ',')` dollar change in 2011 earnings, holding the value of all other $X$ variables constant
...
- $X_{3i}$ (identifies as female); $\hat{\beta_3}=$ `r format(round(summary(intern_mod2)$coefficients[4,1], digits =0),big.mark = ',')`
  - being female as opposed to male is associated with a `r format(round(summary(intern_mod2)$coefficients[4,1], digits =0),big.mark = ',')` dollar change in 2011 earnings, holding the of value all other $X$ variables constant
- $X_{4i}$ (identifies as Asian or Pacific Islander (API)); $\hat{\beta_4}=$ `r format(round(summary(intern_mod2)$coefficients[5,1], digits =0),big.mark = ',')`
  - being API as opposed to white is associated with a `r format(round(summary(intern_mod2)$coefficients[5,1], digits =0),big.mark = ',')` dollar change in 2011 earnings, holding the of value all other $X$ variables constant
- $X_{5i}$ (identifies as Black); $\hat{\beta_5}=$ `r format(round(summary(intern_mod2)$coefficients[6,1], digits =0),big.mark = ',')`
  - being Black as opposed to white is associated with a `r format(round(summary(intern_mod2)$coefficients[6,1], digits =0),big.mark = ',')` dollar change in 2011 earnings, holding the of value all other $X$ variables constant
...
<br>
What is the predicted 2011 earnings for a student who participated in an internship ($X_{1i}=1$), who scored 45 on the high school reading test ($X_{2i}=45$) and who identifies as white, who identifies as male?
...
- Most social science disciplines model binary dependent variables via non-linear regression models
  - most common model for 0/1 dependent variables is logistic regression, but interpretation can be difficult because coefficients measured in "log odds"

<br>
  
  
- Econometric models typically model binary dependent variables using __linear probability model__
  ...
  - It can ONLY be used for binary, categorical dependent variables
    - If more than two categories, then we have to use different models (e.g., multinomial logistic regression)

  
<br>

## Interpretation of $\hat{\beta_k}$

<br>

__"percentage point change" vs. "percentage change"__

- When interpreting $\hat{\beta_k}$ for the linear probability model (below) we refer to a "percentage point change in the probability that $Y=1$"
- "Percentage point change" is percentage *A* minus percentage *B* (or vice versa)
  - Example, using Mexican American Studies (MAS) program offered by Tucson Unified School District
    - Imagine that high school students who participate in the Mexican American Studies (MAS) program have a 68% probability of graduating from high school and students who do not participate in MAS have a 60% probability of graduating from high school. 
    - The "percentage point change" in the probability of graduating from high school associated with participating in MAS is: $68\% - 60\%= 8\%$
- "percentage change" expresses the difference between final and initial values
  - imagine that $Y_1$ and $Y_2$ are two different values of some variable
  - percentage change is calculated as $\frac{Y_2 - Y_1}{Y_1} \times 100$
  - Example:
    - imagine that $Y_1= \$4.00$ is the price of one gallone of gas last year and $Y_2= \$6.00$ is the price of a gallon of gas this year
    - the percentage change is $\frac{Y_2 - Y_1}{Y_1} \times 100 = \frac{6 - 4}{4} \times 100 =50$
    - we would say, the price of gas has increased by 50% from last year to this year
- For interpretation of the linear probability model we use **percentage point change** rather than **percentage change**


Run regression model with participation in internship as Y-variable and following X-variables: 

- high school reading score (continuous), parent income (continuous), sex (dichotomous categorical), race (categorical)
```{r}
intern_mod3 <- lm(formula = as.integer(f2intern0506) ~  bytxrstd + parent_income  + f1sex + f1race_v2, data = df_els_stu_fac %>% filter(f2enroll0506=='yes'))
summary(intern_mod3)
```
<br>
<br>

- Population linear probability regression model (multivariate regression)
  - $Y_i = \beta_0 + \beta_1 X_{1i}+ \beta_2 X_{2i}+ \beta_3 X_{3i}+ \beta_4 X_{4i}+ \beta_5 X_{5i}+ \beta_6 X_{6i}+ \beta_7 X_{7i}+ \beta_8 X_{8i}+ u_i$, where:
    - $Y_i$: dichotomous measure of whether student $i$ in participated in internship/co-op in 2005-06
    - $X_{1i}$: (continuous) high school reading test score
    - $X_{2i}$: (continuous) measure of parental income
    - $X_{3i}$: (dichotomous) student $i$ identifies as female
    - $X_{4i}$: (dichotomous) student $i$ identifies as Asian or Pacific Islander (API)
    - $X_{5i}$: (dichotomous) student $i$ identifies as Black 
    - $X_{6i}$: (dichotomous) student $i$ identifies as Latinx
...
- OLS prediction line
  - without estimates
    - $\hat{Y_i} = \hat{\beta_0} + \hat{\beta_1}X_i + \hat{\beta_2} X_{2i}+ \hat{\beta_3} X_{3i}+ \hat{\beta_4} X_{4i}+ \hat{\beta_5} X_{5i}+ \hat{\beta_6} X_{6i}+ \hat{\beta_7} X_{7i}+ \hat{\beta_8} X_{8i}$
  - with estimates
    - $\hat{Y_i} =$  `r format(round(summary(intern_mod3)$coefficients[1,1], digits =4),big.mark = ',')` + `r format(round(summary(intern_mod3)$coefficients[2,1], digits =4),big.mark = ',')` $\times X_{1i}$ + `r format(round(summary(intern_mod3)$coefficients[3,1], digits =4),big.mark = ',')` $\times X_{2i}$ + `r format(round(summary(intern_mod3)$coefficients[4,1], digits =4),big.mark = ',')` $\times X_{3i}$ +  `r format(round(summary(intern_mod3)$coefficients[5,1], digits =4),big.mark = ',')` $\times X_{4i}$ + `r format(round(summary(intern_mod3)$coefficients[6,1], digits =4),big.mark = ',')`  $\times X_{5i}$ +  `r format(round(summary(intern_mod3)$coefficients[7,1], digits =4),big.mark = ',')` $\times X_{6i}$ + `r format(round(summary(intern_mod3)$coefficients[8,1], digits =4),big.mark = ',')`  $\times X_{7i}$ +  `r format(round(summary(intern_mod3)$coefficients[9,1], digits =4),big.mark = ',')` $\times X_{8i}$
    
<br>

__Interpreting Linear Probability Model, Continuous X__

- Dependent variable is a 0/1 measure of whether student participated in an internship
- __Interpretation of $\hat{\beta_k}$__
  - Generic: 
    - On average, a one-unit increase in X is associated with a $\hat{\beta_k}*100$ percentage point change in the probability of Y=1
  - Interpretation for $X_{1i}$ (high school reading test score); $\hat{\beta_1}=$ `r format(round(summary(intern_mod3)$coefficients[2,1], digits =4),big.mark = ',')`
    - on average, a one-point increase in high school reading test score is associated with a `r format(round(summary(intern_mod3)$coefficients[2,1]*100, digits =4),big.mark = ',')` percentage-point change in the probability of having an internship.
    - that is, this is a decline in probability of receiving an internship that is less than one percentage point.
- __Interpretation of $\hat{\beta_k}$__ when you SCALE X
  - Generic: 
    - On average, a N-unit increase in X is associated with a ((N-unit$*\hat{\beta_1}$)*100) percentage point change in the probability of Y=1
  - Interpretation for a 10-point increase in $X_{1i}$ (high school reading test score); $\hat{\beta_1}=$  `r format(round(summary(intern_mod3)$coefficients[2,1], digits =4),big.mark = ',')`  
    - on average, a 10-point increase in high school reading test score is associated with a `r format(round(summary(intern_mod3)$coefficients[2,1]*100*10, digits =4),big.mark = ',')` percentage-point change in the probability of having an internship.
    - that is, this is about a 1.5 percentage point decline in probability of receiving an internship (e.g., from 50.0% to 48.5% probability of receiving an internship; or going from a 30% to 28.5% chance of receiving an internship.

<br>

__Linear Probability Model, with Categorical X__

- Dependent variable is a 0/1 measure of whether student participated in an internship
- __Interpretation of $\hat{\beta_1}$__
  - Generic: being in the "non-reference group" as opposed to the "reference group" is, on average, associated with  a $100*\hat{\beta_1}$ percentage point change in the probability of Y=1
- Interpretation for $X_{8i}$ (0/1 student identies as two or more races); $\hat{\beta_8}$ `r format(round(summary(intern_mod3)$coefficients[9,1], digits =4),big.mark = ',')`
  - (reference group is "identifies as white")
  - identifying as two-or-more races as opposed to identifying as white is, on avarage, associated with `r format(round(summary(intern_mod3)$coefficients[9,1]*100, digits =4),big.mark = ',')` percentage-point change in the probability of having an internship.
  - that is, this about a 2.5% increase in the probability of having an internship (e.g., from 20% to 22.5% probability of having an internship)

<br><br>

__Interpretation of results from journal articles__
...
Below is partial screenshot from Table 3 from, where:

- $Y$ = 0/1 student graduated from high school
- $X_1$ = 0/1 student participated in the Mexican American Studies (MAS) major

![](mas_table3.png)

<br>

And below is text interpreting results 

- note that the regression coefficient of `0.095` was interpreted as, participating in MAS as opposed to not participating in MAS was, on average, associated with a `9.5` percentage point increase in the probability of graduattion
  - where the `9.5` is calculated as follows: $\hat{\beta_1}*100 = .0095 * 100 = 9.5$

![](mas_table3_interpretation.png)
