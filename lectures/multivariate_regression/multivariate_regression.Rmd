...
# Create a dataframe df_els_stu_fac that has categorical variables as factor class variables rather than labelled class variables
  ...
  for (v in c('bytxmstd','bytxrstd','f1txmstd','f3stloanamt','f3stloanpay','f3ern2011')) {
...
- the red dotted curve is the sampling distribution from an unbiased estimator (e.g., if you used sample mode as the estimator for population mean)
...
<br><br>
ADD TEXT/CODE CHUNK SHOWING HOW TO CALCULATE ROBUST STANDARD ERRORS
```{r}
df_els_stu_fac %>% filter(f3a01d=="No") %>%  ggplot(aes(x=f3totloan, y=f3ern2011)) + geom_point() + stat_smooth(method = 'lm')
