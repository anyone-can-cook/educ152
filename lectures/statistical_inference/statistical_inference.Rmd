...
A small p-Small p-value means that it would be unusual to find the sample estimate we observed if the null hypothesis $H_0$ is find the observed data if 𝐻0were true.

TWEAK THIS TEXT COPIED FROM GOOGLE DOC

–t= value of your t-test
Two-sided hypothesis (𝐻𝑎:𝜇≠𝜇0)
Pr(obs>t) + pr(obs<-t)

CALCULATE T-TEST FOR SAMPLE AND ONE H_0; INTERPRET P-VALUE
...
PLAYING WITH T-TEST FUNCTION AND PLOT_T_DISTRIBUTION FUNCTION CRYSTAL CREATED
```{r}
...
mean(df_ipeds_sample$coa_grad_res, na.rm = TRUE)
t.test(df_ipeds_sample$coa_grad_res, mu = 28000)
plot_t_distribution(df_ipeds_sample$coa_grad_res, mu = 28000)
```

Experiment with plotting population distribution on top of single sample distribution on top of sampling distribution assuming H_0
```{r}
plot_distribution(df_ipeds_pop$coa_grad_res, plot_title = 'Population distribution') +
  plot_distribution(df_ipeds_sample$coa_grad_res, plot_title = 'Single sample distribution') +
  plot_t_distribution(df_ipeds_sample$coa_grad_res, mu = 28000, plot_title = 'Sampling distribution, assuming H_0') +
...
Experiment with single sample distribution on top of sampling distribution assuming H_0
```{r}
  plot_distribution(df_ipeds_sample$coa_grad_res, plot_title = 'Single sample distribution') +
  plot_t_distribution(df_ipeds_sample$coa_grad_res, mu = 28000, plot_title = 'Sampling distribution, assuming H_0') +
  plot_layout(ncol = 1)
...
Experiment with plotting population distribution on top of single sample distribution on top of sampling distribution
```{r}
plot_distribution(df_ipeds_pop$coa_grad_res, plot_title = 'Population distribution') +
  plot_distribution(df_ipeds_sample$coa_grad_res, plot_title = 'Single sample distribution') +
  plot_distribution(get_sampling_distribution(df_ipeds_pop$coa_grad_res), plot_title = 'Sampling distribution') +
