...
plot_t_distribution <- function(data_vec, mu, alpha = 0.05, alternative = 'two.sided', plot_title = '', shade_rejection = T, shade_pval = T) {
  ...
  if (alternative != 'greater') {
    ...
    if (shade_rejection) {
      p <- p + geom_area(aes(fill = 'cval'), alpha = 0.3,
                         stat = 'function', fun = function(x) dt(x, df = deg_freedom),
                         xlim = c(-4, cv_lower))
    }
    
    if (shade_pval) {
      p <- p + geom_area(aes(fill = 'tstat'), alpha = 0.3,
                         stat = 'function', fun = function(x) dt(x, df = deg_freedom),
  ...
  p <- p +
    ...
    theme(plot.title = element_text(size = 10, face = 'bold', hjust = 0.5),
          legend.title = element_text(size = 9, face = 'bold'),
          legend.text = element_text(size = 8),
          legend.box.background = element_rect(color = NA, linetype = 0))
