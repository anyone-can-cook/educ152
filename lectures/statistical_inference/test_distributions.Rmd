...
plot_distribution <- function(data_df, data_var, group_var = NULL, group_cat = NULL, show_group_hist = F, plot_title = '') {
  ...
  if (!two_pop || show_group_hist) {  # show histogram only if 1 pop or show_group_hist is TRUE
    p <- p +
  ...
  if (two_pop) {
    ...
    if (!two_pop || show_group_hist) {  # show histogram only if show_group_hist is TRUE
      p <- p +
...
# IPEDS data: tuitfee_grad_res by control, custom categories/order
