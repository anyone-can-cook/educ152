...
- Total Sum of Squares (TSS) = $\sum_{i=1}^{n} (Y_i-\bar{Y})^2$
  - A measure of the total variance in Y, relative to $\bar{Y}$
  - total sum of squares = `r format(round(sum((df_socialwork$debt_all_stgp_eval_mean - ybar_i)^2, na.rm = TRUE), digits=0),big.mark = ',')`
- Explained Sum of Squares (ESS) = $\sum_{i=1}^{n} (\hat{Y_i}-\bar{Y})^2$
  - Difference between predicted values $\hat{Y_i}$ and sample mean $\bar{Y}$
  - Measures amount of variation in Y explained by X
  - explained sum of squares = `r format(round(sum((yhat_i - ybar_i)^2, na.rm = TRUE), digits=0),big.mark = ',')`
- Sum of Squared Residuals (SSR) = $\sum_{i=1}^{n} (Y_i- \hat{Y_i})^2$
  - Difference between actual value observed and predicted value by our regression line
  - Measures amount of variation in Y not explained by X
...
- $R^2 = \frac{\text{variance in Y that is explained by X}}{\text{total variance in Y}} =  \frac{ESS}{TSS}=$ (`r format(round(sum((yhat_i - ybar_i)^2, na.rm = TRUE), digits=0),big.mark = ',')`)/(`r format(round(sum((df_socialwork$debt_all_stgp_eval_mean - ybar_i)^2, na.rm = TRUE), digits=0),big.mark = ',')`) = `r format(round(sum((yhat_i - ybar_i)^2, na.rm = TRUE)/sum((df_socialwork$debt_all_stgp_eval_mean - ybar_i)^2, na.rm = TRUE), digits=4),big.mark = ',')`
...
```
### Student exercise about $R^2$ [PATRICIA PLEASE MODIFY]

PATRICIA - MODIFY SO THAT REGRESSION MODEL IS RELATIONSHIP BETWEEN COST OF ATTENDANCE (X) AND EARNINGS (Y); earnings var is `df_socialwork$earn_mdn_hi_2yr`
...
__Student Exercise #2 [SOLUTIONS]__
RQ1: What is the relationship between district-wide math test scores (Y) and district expenditures per student (X)?
...
### SER student exercise [PATRICIA MODIFY]

PATRICIA - MODIFY SO THAT REGRESSION MODEL IS RELATIONSHIP BETWEEN COST OF ATTENDANCE (X) AND EARNINGS (Y); earnings var is `df_socialwork$earn_mdn_hi_2yr`

__Student Exercise #3__
...
__Student Exercise #3 [SOLUTIONS]__
RQ1: What is the relationship between district-wide math test scores (Y) and district expenditures per student (X)?
...
- $SE(\hat{\beta}_1)$, the sample standard error of $\hat{\beta}_1$ is an estimate of how far away, on average, the value of $\hat{\beta}_1$ from a single random sample is from the value of the expected value $E(\hat{\beta}_1)$, which  is the mean value of $\hat{\beta}_1$ from an infinite number of random sampples
...
## Student Exercise [PATRICIA MODIFY]
