...
```{r}
df_score_ipeds %>% count(locale)
df_score_ipeds %>% count(urban)

attributes(df_score_ipeds$locale)
attr(x = df_score_ipeds$locale, which = 'labels')

df_score_ipeds %>% count(locale) %>% as_factor()

df_score_ipeds %>% mutate(
  urban = case_when(
    locale %in% c(11,12,13) ~ 1, # city
    locale %in% c(21,22,23) ~ 2, # suburb
    locale %in% c(31,32,33,41,42,43) ~ 3 # town/rural
  )
) 

var_label(df_ipeds_pop[['urban']]) <- 'three-category variable of level of urbanization (city, suburb, town/rural) created from locale'
# %>% count(urban, locale) # crosscheck
...
$ opeid6                        <chr> "001002", "001052", "025034", "001055", "001051...
$ unitid                        <dbl> 100654, 100663, 100690, 100706, 100751, 100858,...
$ instnm                        <chr> "Alabama A & M University", "University of Alab...

# already factor
$ control                       <fct> Public, Public, Private not-for-profit, Public,...
$ ccbasic                       <fct> Master's Colleges & Universities: Larger Progra...
$ region                        <fct> "Southeast (AL, AR, FL, GA, KY, LA, MS, NC, SC,...
$ locale                        <fct> "City: Midsize (population of at least 100,000 ...
$ credlev                       <fct> Master's Degree, Master's Degree, Master's Degr...


$ stabbr                        <chr> "AL", "AL", "AL", "AL", "AL", "AL", "AL", "AL",...
$ city                          <chr> "Normal", "Birmingham", "Montgomery", "Huntsvil...
$ cipdig2                       <chr> "52", "52", "52", "52", "52", "52", "52", "52",...
$ cipcode                       <chr> "5202", "5202", "5202", "5202", "5202", "5202",...
$ cipdesc                       <chr> "Business Administration, Management and Operat...
$ creddesc                      <chr> "Master's Degree", "Master's Degree", "Master's...
...
Investigate the attributes of `df_mba$s3region`
```{r, results="hide"}
typeof(df_mba$region)
...
attr(df_mba$region,"class")

attr(df_mba$region,"labels")
```        
### `factor` object class
