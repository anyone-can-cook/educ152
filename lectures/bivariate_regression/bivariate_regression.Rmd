...
- $Y_i$ = institution-level student debt at university i
...
- __Population Linear Regression Model__
  ...
  - where:
    ...
    - $Y_i$ = institution-level student debt at university $i$
    - $X_i$ = annual cost of attendance for full-time resident graduate students at university $i$
    - $\beta_0$ = population intercept"), which represents average value of $Y$ (institution-level student debt) at a university with X=0 (cost of attendance = 0)
  - $\beta_1$ = population regression coefficient, which represents average effect of a one-unit increase in $X$ on the value of $Y$
- __OLS Prediction Line or "OLS Regression Line" (based on sample data)__
   - $\hat{Y_i} = \hat{\beta_0} + \hat{\beta_1}X_i$   
...
## Exercise [PATRICIA - MODIFY]

RQ: What is the effect of age on XXXX?? 
PATRICIA - MAYBE MODIFY THIS SO NOW THE DEPENDENT VARIABLE IS EARNINGS TWO YEARS AFTER GRADUATION INSTEAD OF DEBT
